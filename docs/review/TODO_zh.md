...
- [x] **P1-2 轮子速度伺服的行为验收** → 已迁至 `DONE_zh.md` 第十四节（`--mode wheel_step`：四轮稳态 +5.00 rad/s，命令 +5）
  ...
  - 验收（已达成）：`--mode wheel_step`（阶跃 +5 rad/s）→ 四个轮子稳态
    **+5.00 +5.00 +5.00 +5.00 rad/s**（误差 < 0.2%），上升时间约 1 s（力矩限幅内加速整机）。
